...
struct WebSearchMcpExtensionSettings {
    /// The API key for z.ai web search service.
    #[serde(default)]
    zai_api_key: Option<String>,
}

struct WebSearchMcpExtension;

impl zed::Extension for WebSearchMcpExtension {
    fn new() -> Self {
        Self
    }
    ...
    fn context_server_command(
        ...
        let settings = ContextServerSettings::for_project("mcp-server-zai-web-search", project)?;

        let settings_struct: WebSearchMcpExtensionSettings = match settings.settings {
            Some(value) => serde_json::from_value(value).map_err(|e| format!("{e}"))?,
            None => WebSearchMcpExtensionSettings { zai_api_key: None },
        };
    ...
    fn context_server_configuration(
        ...
        if let Ok(user_settings) = settings {
            if let Some(settings_value) = user_settings.settings {
                if let Ok(web_search_settings) =
                    serde_json::from_value::<WebSearchMcpExtensionSettings>(settings_value)
                {
                    match web_search_settings.zai_api_key {
                        Some(zai_api_key) => {
        ...
        let settings_schema =
            serde_json::to_string(&schemars::schema_for!(WebSearchMcpExtensionSettings))
                .map_err(|e| format!("{e}"))?;
...
zed::register_extension!(WebSearchMcpExtension);
